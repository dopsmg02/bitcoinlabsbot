...
try {
    ...
    const sshCommand = `ssh -i "C:\\\\d\\\\cologi2025@outlook.com.pem" -o StrictHostKeyChecking=no ubuntu@3.0.48.97 "echo ${mineBase64} | base64 --decode > ~/bitcoin-labs_production/server/src/controllers/mine.controller.ts && echo ${antiCheatBase64} | base64 --decode > ~/bitcoin-labs_production/server/src/middlewares/antiCheat.middleware.ts && cd ~/bitcoin-labs_production/server && npm run build && pm2 restart bitcoin-labs-api"`;
